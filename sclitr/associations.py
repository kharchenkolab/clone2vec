...
def _fast_corr(
    ...
    if not sp.issparse(X):
        ...
        # Precompute std_Y once and allocate S for batched slope computation
        if slope:
            std_Y = np.std(Y_data, axis=0)
            S = np.full_like(R, np.nan)

        # Precompute significance params and allocate P for batched p-values
        if significance:
            n = X.shape[0]
            P = np.full_like(R, np.nan)
            df = n - 2
...
def associations(
    ...
    **gam_kwargs : dict, optional
    ...
    for i, (X, X_name) in enumerate(list(zip(Xs, X_names))):
        if method in ["pearson", "spearman"]:
            X_res = _fast_corr(X, Y[masks[i]], method=method, significance=True, progress_bar=progress_bar)
        else:
            X_res = _gam(X, Y[masks[i]], random_state=random_state, progress_bar=progress_bar, **gam_kwargs)
